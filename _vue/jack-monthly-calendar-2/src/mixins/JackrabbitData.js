export default {
    ...
    methods: {
        ...
        isClassAvailableForDate(row, date) {
            ...
            //if (row.name == 'Fort Mill Elementary School Fall 2023 After School Enrichment Program') {
                //console.log(row)
                //console.log(date)
            ...
            // Start date
            if (row.start_date && row.start_date == dateFormat)
            {
                return true
            }
        ...
        getListOfValuesForField(term, rows = null) {
            ...
            if ( field == 'age')
            {
                ...
                return {
                    ...
                    list_values_display: list_values.sort().map(value => {
        ...
        getClassesFor(date) {
            //console.log('>>>> getClassesFor ', date)
            let dateFormat = date.dateFormat
        ...
        cumpleElFiltro(row, value_selected, filter)
        {
            ...
            if (filter.location_filter) {
                if (row[filter.field].indexOf(value_selected) == -1) return false
            }
    ...
    // NO FUNCIONA BIEN, MEJOR NO USARLO
    // Cambiar los otros filtros en función del filtro cambiado
    /*watch: {
        filter0_selected(newValue) {
            console.log('Filter 0 selected: ' + newValue)
            // Actualizar los filtros 1 y 2

            if (newValue == null) {
                //console.log('Restutiyendo los filtros 1 y 2')
                if (typeof this.filters[1] != 'undefined') {
                    this.filters[1].values_allowed = JSON.parse(JSON.stringify(this.filters[1].values_allowed))
                }
                if (typeof this.filters[2] != 'undefined') {
                    this.filters[2].values_allowed = JSON.parse(JSON.stringify(this.filters[2].values_allowed))
                }
            }
            else {
                // Prepara el primer filtro
                //console.log('Preparando los filtros porque filtro 0 = ' + newValue)
                const filter_fields = {}
                filter_fields[this.filters[0].field] = this.filter0_selected

                if (typeof this.filters[1] != 'undefined') {
                    this.filters[1].values_allowed = this.getListOfValuesForField(this.filters[1].field, this.getRowsWitchTheseFields(filter_fields))
                }
                if (typeof this.filters[2] != 'undefined') {
                    this.filters[2].values_allowed = this.getListOfValuesForField(this.filters[2].field, this.getRowsWitchTheseFields(filter_fields))
                }
                //console.log(this.filters[1].values_allowed)
                //console.log(this.filters[2].values_allowed)
            }

            this.filter1_selected = null
            this.filter2_selected = null
        },
        filter1_selected(newValue) {
            console.log('Filter 1 selected: ' + newValue)

            if (newValue == null) {
                //console.log('Restutiyendo los filtros 2')
                if (typeof this.filters[2] != 'undefined') {
                    this.filters[2].values_allowed = JSON.parse(JSON.stringify(this.filters[2].values_allowed))
                }
            }
                ...
                const filter_fields = {}
                filter_fields[this.filters[0].field] = this.filter0_selected
                filter_fields[this.filters[1].field] = this.filter1_selected
        ...
        filter2_selected(newValue) {
            console.log('Filter 2 selected: ' + newValue)
        }
    }*/
}
